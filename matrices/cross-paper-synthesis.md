...
P04 uses human PR feedback as a benchmark target. P05 uses manually verified PRs and structured ground-truth reports. P03 uses developer behavior and feedback in production. P06 constructs a large benchmark through issue/PR linkage and multi-stage filtering. P07 uses live reviewer acceptance and value judgments. These signals are realistic, but they can still miss valid AI comments or mix correctness with priority, preference, style, trust, workload, and team norms.

**Implication for us:** Human review feedback and benchmark ground truth should be treated as imperfect evidence, not absolute truth. Structured issue reports, filtering pipelines, and live user studies improve reliability, but they still require annotation-quality and construct-validity analysis.

### 6. Usefulness is not the same as direct acceptance

P07 is especially important because it separates direct acceptance from perceived value and downstream revisions. Some generated comments are not accepted directly but are still marked valuable as review or development tips. This suggests that a binary accepted/rejected metric may undercount practical usefulness.

**Implication for us:** Our framework should distinguish acceptance, usefulness, actionability, correctness, and downstream impact.

## Emerging Gap Statement

The first seven papers show that the field is improving its evaluation of LLM-based code review: moving beyond lexical similarity, introducing richer rubrics, detecting hallucinations, evaluating real deployments, building PR-level benchmarks, adding full-project-context benchmarks, enriching context with issue/PR and surrounding-code information, and measuring live reviewer behavior. However, they still do not provide a unified framework that jointly evaluates:
...
- actionability, usefulness, acceptance, and downstream impact,
...
A promising contribution is a **taxonomy and trade-off-aware evaluation framework** for LLM-generated code review comments, with special emphasis on context quality, data quality, human-centered value, and the consequences of filtering, aggregating, or suppressing generated comments.

## Paper-to-Argument Mapping

| Argument Need | Best Supporting Papers | Notes |
|---|---|---|
| Text similarity is insufficient | P01, P03, P04, P05, P06, P07 | P01 is strongest for direct critique; P03–P07 add practical, benchmark, fine-grained, and live-user realism. |
...
| Context quality matters | P02, P03, P04, P05, P06, P07 | P04 shows more context may hurt; P05 shows full context matters; P06 shows semantic/textual/code context enrichment matters; P07 shows RAG context in live review. |
| Current work lacks trade-off-aware filtering/aggregation analysis | P01, P02, P03, P04, P05, P06, P07 | All touch parts of the problem, none fully model useful-comment preservation, cost, and filtering/aggregation consequences. |
...
| Live user-study metrics matter | P03, P07 | P03 gives production workflow metrics; P07 gives acceptance, value, overhead, and downstream revision signals. |
| Production value needs workflow metrics | P03, P06, P07 | P03 is the strongest industrial anchor; P06 adds ByteDance deployment evidence; P07 adds Mozilla/Ubisoft live study evidence. |
| Benchmark ground truth is incomplete | P04, P05, P03, P06, P07 | Human PR feedback, structured issue reports, production reactions, and reviewer acceptance/value signals are useful but noisy. |
...
| Usefulness must be separated from acceptance | P07, P03 | P07 is strongest; P03 supports practical usefulness through code resolution. |
...
This supports our central framing: the question is not whether to add more context, but how to evaluate, select, and filter high-quality context under cost, data-quality, and attention constraints.
...
This supports another core framing: a generated review comment can be useful even if it is not directly accepted, and accepted comments are not automatically correct.

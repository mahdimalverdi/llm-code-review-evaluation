...
P14 already recognized that BLEU is weak for review comments because valid comments are diverse and non-unique. Later work adds richer dimensions: grounding (P02), usefulness (P01/P18/P23/P24/P25), security category/location (P21), downstream refinability (P24), suggestion adoption and code impact (P26), signal-to-noise and abandonment (P27), and coding preferences (P30). P29/P31/P32 then show that replacing lexical metrics with LLM judges is not enough unless the judge itself is validated.

**Implication:** Our framework should not be another single metric. It should be a structured evaluation protocol with separate content, grounding, usefulness, workflow, and evaluator-validity dimensions.

### 2. Context quality is broader than code context

The literature moves from diff hunks (P14/P15) to PR/repository context (P04/P05/P06), retrieved exemplars (P11/P20), specifications (P12), semantic metadata (P13), static-analysis output (P22), reviewer provenance (P23), structured reasoning chains (P25), socio-technical workflow context (P26/P28), and evaluator context (P29/P31/P32/P33/P36).

**Implication:** Context quality should include code, retrieval, static-analysis, specification, reviewer/provenance, downstream repair, socio-technical workflow, and evaluator-prompt context.

### 3. Data, reference, and provenance quality are first-class concerns

P08/P18 show that human review comments can be noisy. P17 shows strict curation improves reliability but reduces coverage. P22 shows synthetic/static-analysis signals can help but may contradict. P23 shows reviewer experience affects reference quality. P25 restructures sparse comments into issue-location-explanation-repair chains. P30 shows preference data scales with LLM judges but becomes judge-dependent. P31/P32 show carefully constructed benchmarks expose failures hidden by easier setups. P35 shows known reference bugs improve agreement and make critique evaluation more objective.

**Implication:** Evaluation should record who/what produced the reference, how complete it is, whether it targets a known issue, and whether multiple valid comments exist.

### 4. Taxonomy must cover comment failures, workflow failures, and evaluator failures

The taxonomy should include hallucination, irrelevance, vagueness, non-actionability, wrong location, wrong category, severity miscalibration, redundancy, uncivil/unclear comments, missing explanation, missing repair suggestion, low-signal CRA feedback, complexity-increasing suggestions, automation that removes knowledge transfer, and evaluator failures such as position bias, verbosity bias, prompt sensitivity, invalid answer format, source-model sensitivity, preprocessing sensitivity, authority/sentiment/distraction/refinement/self-enhancement bias, and adversarial judge attacks.

**Implication:** The final taxonomy should explicitly separate **generated-review failures** from **evaluation-instrument failures**.

### 5. Mitigation strategies are trade-off choices, not free improvements

Mitigations include filtering (P10), data cleaning (P08/P18), RAG (P11/P16/P20), specification grounding (P12), PEFT/fine-tuning (P13/P15), multi-agent generation (P09), prompt routing (P21), static-analysis hybrids (P22), reviewer-experience weighting (P23), reward optimization (P24), CoT curation (P25), human oversight (P26/P28), signal-to-noise monitoring (P27), thinking judges (P31), bias auditing (P32), tool/human-augmented judges (P33/P34/P36), and critic-assisted human oversight (P35).
...
Most works measure improved scores or reduced bad comments, but few measure lost useful comments. Precision-first filters may suppress rare but valuable issues (P10/P21). Cleaning/reformulation may change intent (P18/P25). Reward models may optimize model-easy usefulness (P24). AI suggestions may be adopted but increase complexity (P26). CRA-only review may be present but low-signal (P27). Automation may save time but remove socio-technical benefits (P28). Bias-audited judges may reject inconsistent cases, but this changes coverage (P32/P36).
...
The first thirty-six papers show that LLM-based code review evaluation is moving from lexical similarity toward richer rubrics, PR-level benchmarks, context enrichment, production telemetry, data curation, RAG, specification grounding, static-analysis hybrids, reviewer provenance, reward-guided generation, structured comprehensibility, human-AI workflow analysis, signal-to-noise measures, and judge robustness benchmarks. However, they still do not provide a unified framework that jointly evaluates:
...
- context quality: diff, PR, repository, retrieval, specification, static-analysis, semantic metadata, reviewer provenance, and socio-technical workflow context,
- reference quality: completeness, provenance, reviewer expertise, known-issue anchoring, multiple-valid-comment cases, and annotation reliability,
- workflow value: acceptance, adoption, abandonment, signal-to-noise, reviewer effort, knowledge transfer, team awareness, shared ownership, and accountability,
- useful-feedback preservation under filtering, cleaning, routing, retrieval, reward optimization, static-analysis hybridization, and judge-based suppression,
...
- proxy-validity limits of BLEU, ROUGE, BERTScore, EM, CodeBLEU, LLM-judge scores, Outdated Rate, acceptance, adoption, signal-to-noise, abandonment, and code metric deltas,
- cost: token budget, latency, training cost, API cost, judge cost, routing cost, reward-model cost, expert-maintenance cost, reviewer overhead, and human-AI interaction cost.

## Working Contribution Direction

A strong contribution is a **taxonomy and trade-off-aware evaluation framework** for LLM-generated code review comments. The framework should jointly cover context quality, reference provenance, structured review outputs, downstream usefulness, socio-technical review value, evaluator validity, problematic-comment types, useful-feedback preservation, proxy-validity analysis, and the consequences of filtering, cleaning, prompting, fine-tuning, retrieving, routing, rewarding, judging, verifying, aggregating, suppressing, blocking, or expanding generated comments.

## Paper-to-Argument Mapping

| Argument Need | Best Supporting Papers | Notes |
|---|---|---|
| Text similarity is insufficient | P14, P01, P11, P13, P16, P17, P18, P20, P21, P22, P23, P24, P25, P26, P29, P30, P31 | LLM judges do not automatically solve metric validity. |
| Context quality matters | P04, P05, P06, P10, P11, P12, P13, P16, P20, P21, P22, P23, P25, P26, P28, P29, P31, P32 | Includes evaluator context. |
| Data/reference quality matters | P08, P14, P17, P18, P20, P22, P23, P25, P30, P31, P35 | P35 adds known-bug anchoring. |
| Taxonomy/usefulness categories matter | P01, P02, P08, P10, P18, P19, P21, P23, P25, P26, P27, P28, P30, P35 | Include comment and workflow categories. |
| Hybrid mitigation needs trade-off analysis | P09, P10, P11, P12, P16, P20, P21, P22, P24, P25, P35 | P35 adds precision/recall-like critique frontier. |
| Downstream usefulness matters | P07, P10, P20, P24, P25, P26, P27, P35 | P35 adds human-assisted bug catching. |
| Structured output matters | P17, P21, P25, P30, P35 | Issue, location, explanation, repair, known-bug inclusion. |
| LLM-as-a-Judge needs calibration/robustness | P16, P18, P22, P24, P29, P30, P31, P32, P33, P34, P36 | Strongest evidence now P29–P36. |
| Bias/adversarial threat model matters | P31, P32, P33, P36 | P32 is SE-specific; P36 is broad taxonomy. |
| Human-AI workflow matters | P03, P07, P10, P12, P21, P26, P27, P28, P35 | P35 adds human+critic teams. |
| Useful-feedback preservation is missing | P10, P11, P12, P18, P20, P21, P22, P23, P24, P25, P26, P27, P28, P32 | Include coverage loss under judge filtering. |
...
- **P22:** Static-analysis context helps only when integrated carefully.
- **P28:** Review context includes people, rationale, process, and learning goals.
- **P29/P31/P32:** Evaluator context itself changes judgments.
...
- **P28:** Automation may remove knowledge transfer and shared ownership.

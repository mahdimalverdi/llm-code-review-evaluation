...
\begin{figure}[H]
\centering
\input{../figures/related-work-coverage-map.tex}
...
\begin{figure}[H]
\centering
\input{../figures/corpus_assembly_flow.tex}
\caption{Unified corpus assembly. Overlapping multi-source search streams were reconciled into 121 included studies. The amendment contributed no new studies: 42 records matched existing corpus studies, while 1,316 unmatched candidates remained outside the synthesis denominator.}
...
\toprule
\rowcolor{ACMTableHead}
\textbf{Field} & \textbf{Raw agreement} & \textbf{Cohen's kappa} \\
\midrule
...
\subsubsection{Protocol deviations}
...
\toprule
\rowcolor{ACMTableHead}
\textbf{Item} & \textbf{Initial protocol} & \textbf{Executed procedure} & \textbf{Reason} & \textbf{Potential effect} \\
\midrule
\endfirsthead
\toprule
\rowcolor{ACMTableHead}
...
\begin{figure}[H]
\centering
\input{../figures/rq4_tradeoff_evidence.tex}
...
\begin{figure}[H]
\centering
\input{../figures/taxonomy_label_architecture.tex}
...
Mitigation studies often report quality gains without consistently reporting or evaluating useful-feedback preservation, review or issue coverage, human workload, or computational cost. This paper proposes a taxonomy and six-layer framework for recording these outcomes and supporting decisions to show, suppress, rewrite, or escalate feedback.

The later multi-source search amendment produced a large auditable candidate pool, but these records have not undergone complete eligibility assessment and extraction. The synthesis findings therefore remain limited to the frozen 121-study corpus. Independent annotation and controlled studies remain necessary before the proposed taxonomy and framework can support empirical or deployment claims.
